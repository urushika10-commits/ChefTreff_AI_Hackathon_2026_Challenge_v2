...
const SPECS_CONTEXT_HEADER = (specs: string) =>
  specs.trim()
    ? `\n\n---\n## Project Specifications / Requirements\n\nThe following requirements were provided at the start of this session. Treat them as the authoritative source of truth for what this project must do:\n\n${specs.trim()}\n---\n`
    : ''
...
export function buildSystemPrompt(modeId: ModeId, role: Role, repoContext: string, specs?: string): string {
  const context  = REPO_CONTEXT_HEADER(repoContext)
  const specsCtx = SPECS_CONTEXT_HEADER(specs ?? '')
...
Specific technical limits, precision requirements, and regulatory constraints.${context}`,
  ...
  return prompts[modeId][role] + specsCtx + MATH_FORMATTING
}
